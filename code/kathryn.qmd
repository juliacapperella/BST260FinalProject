...
# Plots of Waves
dat_waves <- dat |>
  mutate(wave = case_when(between(date, make_date(2020, 3, 1), make_date(2020, 6, 30)) ~ 1,
                          between(date, make_date(2020, 7, 1), make_date(2020, 8, 31)) ~ 2,
                          between(date, make_date(2020, 9, 1), make_date(2021, 2, 28)) ~ 3,
                          between(date, make_date(2021, 3, 1), make_date(2021, 6, 30)) ~ 4,
                          between(date, make_date(2021, 7, 1), make_date(2021, 12, 31)) ~ 5,
                          between(date, make_date(2022, 1, 1), make_date(2022, 3, 31)) ~ 6,
                          between(date, make_date(2022, 4, 1), make_date(2022, 10, 31)) ~ 7,
                          between(date, make_date(2022, 11, 1), make_date(2023, 7, 31)) ~ 8,
                                  TRUE ~ NA)
...
# WAVE 1
wave_1_dr <- dat_waves |>
  filter(wave == 1, !is.na(deaths)) |>
  group_by(state) |>
  summarize(death_rate_per_100k = sum(deaths, na.rm = TRUE) / mean(population, na.rm = TRUE) * 100000)

wave_1_dr <- dat_waves |>
  filter(wave == 1, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 2
wave_2_dr <- dat_waves |>
  filter(wave == 2, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 3
wave_3_dr <- dat_waves |>
  filter(wave == 3, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 4
wave_4_dr <- dat_waves |>
  filter(wave == 4, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 5
wave_5_dr <- dat_waves |>
  filter(wave == 5, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 6
wave_6_dr <- dat_waves |>
  filter(wave == 6, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 7
wave_7_dr <- dat_waves |>
  filter(wave == 7, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))

# WAVE 8
wave_8_dr <- dat_waves |>
  filter(wave == 8, !is.na(deaths)) |> 
  group_by(state) |>
  summarize(death_rate_per_100k = mean(deaths / population * 100000))
...
wave_6_dr |>
  ggplot(aes(x = reorder(state, death_rate_per_100k), y = death_rate_per_100k)) +  # reorder state, assign aesthetic mapping
  geom_col(position = "dodge") + # use geom_col to plot barplot
  labs(title = "Covid-19 Death Rate by State for Wave 6",
       x = "State", 
       y = "Weekly Death Rate per 100k") +
  coord_flip()  # flip coordinates
...
wave_7_dr |>
  ggplot(aes(x = reorder(state, death_rate_per_100k), y = death_rate_per_100k)) +  # reorder state, assign aesthetic mapping
  geom_col(position = "dodge") + # use geom_col to plot barplot
  labs(title = "Covid-19 Death Rate by State for Wave 7",
       x = "State", 
       y = "Weekly Death Rate per 100k") +
  coord_flip()  # flip coordinates
...
wave_8_dr |>
  ggplot(aes(x = reorder(state, death_rate_per_100k), y = death_rate_per_100k)) +  # reorder state, assign aesthetic mapping
  geom_col(position = "dodge") + # use geom_col to plot barplot
  labs(title = "Covid-19 Death Rate by State for Wave 8",
       x = "State", 
       y = "Weekly Death Rate per 100k") +
  coord_flip()  # flip coordinates
```
