...
$$ $$
\mbox{population prediction} = population_{2021}*(1+ \mbox{growth_rate})^{(\mbox{years since 2021)}}
$$
...
dat_waves |> filter(wave == 1, !is.na(cases), !is.na(hosp), !is.na(covid_deaths)) %>%
  ...
  ggplot(aes(x = reorder(state, deaths_per_cases), y = deaths_per_cases)) +  # reorder state, assign aesthetic mapping
  geom_col(position = "dodge") + # use geom_col to plot barplot
  labs(title = "Deaths Per Cases Per State for Wave 1",
       x = "State", 
       y = "Deaths Rate / Case Rate") +
  coord_flip()  # flip coordinates
...
dat_waves |> filter(wave == 1, !is.na(cases), !is.na(hosp), !is.na(covid_deaths)) %>%
  ...
  mutate(hospitalizations_per_cases = hosp_rate / case_rate) %>% # calculate hosp/cases
  mutate(deaths_per_hospitalizations = death_rate / hosp_rate) %>%
  ggplot(aes(x = reorder(state, hospitalizations_per_cases), y = hospitalizations_per_cases)) +  # reorder state, assign aesthetic mapping
  geom_col(position = "dodge") + # use geom_col to plot barplot
...
dat_waves |> filter(wave == 1, !is.na(cases), !is.na(hosp), !is.na(covid_deaths)) %>%
  ...
  mutate(hospitalizations_per_cases = hosp_rate / case_rate) %>% # calculate hosp/cases
  mutate(deaths_per_hospitalizations = death_rate / hosp_rate) %>%
  ggplot(aes(x = reorder(state, deaths_per_hospitalizations), y = deaths_per_hospitalizations)) +  # reorder state, assign aesthetic mapping
  geom_col(position = "dodge") + # use geom_col to plot barplot
